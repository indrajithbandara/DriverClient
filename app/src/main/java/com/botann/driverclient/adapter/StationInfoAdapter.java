 ...
 */
public class StationInfoAdapter extends BaseAdapter<StationInfo> {
    ...
    public StationInfoAdapter(BaseAdapter adapter, OnLoad onLoad) {
        mAdapter = adapter;
        mOnLoad = onLoad;
    }
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if(viewType == R.layout.footer_bottom_tip) {
            ...
            return new StationInfoAdapter.NoMoreItemVH(view);
        }else if(viewType == R.layout.station_footer) {
            View view = LayoutInflater.from(parent.getContext()).inflate(viewType, parent, false);
            return new StationInfoAdapter.LoadingItemVH(view);
        }else{
    ...
    public int getItemViewType(int position) {
        if(position == getItemCount() - 1) {
            if(hasMoreData) {
                return R.layout.station_footer;
            }else{
